...
export function useAccessibilityCheck() {
  ...
  const checkTouchTargetSize = useCallback(
...
export function useAriaLabel() {
  ...
  const generateScoreLabel = useCallback(
    (score: number, par: number): string => {
  ...
  const generateStatsLabel = useCallback(
    (title: string, value: any, subtitle?: string): string => {
